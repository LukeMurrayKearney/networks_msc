...
def plot_age_mat2(age_mat, age_mat2, buckets):
    ...
    sns.heatmap(age_mat, annot=True,cmap='seismic',square=True, cbar=True,ax=ax[0],fmt=".2f")
    ...
    # ax.collections[0].set_clim(0,np.max(age_mat))
    ax[0].collections[0].set_clim(0,max(np.max(age_mat),np.max(age_mat2)))
    ax[0].set_title("Period 1")

    age_mat2 = np.flip(age_mat2, axis=0)
    sns.heatmap(age_mat2, annot=True,square=True, cbar=True,cmap='seismic',ax=ax[1],fmt=".2f")
    ...
    # ax.collections[0].set_clim(0,np.max(age_mat))
    ax[1].collections[0].set_clim(0,max(np.max(age_mat),np.max(age_mat2)))
    ax[1].set_title("Period 2")
    fig.tight_layout()
    fig.savefig("../../../../Presentation/error_heatmap.png",bbox_inches="tight")
    plt.show()
